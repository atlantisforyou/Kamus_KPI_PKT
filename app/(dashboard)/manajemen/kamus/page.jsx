...
export default function KamusPage() {
  ...
  const fetchKamus = async () => {
    setUi(p => ({ ...p, ldL: true }));
    try { 
      const r = await fetch('/api/kamus?mine=true'); 
...
const handleSubmit = async (statusBtn) => {
    ...
    if (finalStatus === 'reviewed') {
        if (!confirm('Ajukan Kamus KPI ini ke SVP Kompartemen untuk di-approve?')) return;
    }

    setUi(p => ({ ...p, ldF: true, err: '', msg: '' }));
    try {
      const reqFields = ['perspektif_bsc','sasaran_strategis','nama_kpi','definisi_kpi','tujuan_kpi','tipe_kpi','formula_penilaian','jenis_pengukuran','polaritas','frekuensi','sumber_data','satuan','validitas'];
      const miss = reqFields.filter(k => !form[k]);
      if (miss.length > 0) throw new Error('Harap lengkapi semua field wajib bertanda *');
      ...
      if (ui.id) {
        url = `/api/kamus/${ui.id}`;
        method = 'PUT';
        bodyData = { ...form, status: finalStatus }; 
      } else {
        url = '/api/kamus';
        method = 'POST';
        bodyData = { ...form, status: finalStatus };
      ...
      setUi(p => ({ ...p, msg: 'Berhasil disimpan!', ldF: false }));
      setTimeout(() => { goBack(); fetchKamus(); }, 1500);
