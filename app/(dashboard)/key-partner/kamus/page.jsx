...
const Ico = {
  ...
  Succ:   <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
};

const CSS = `*{box-sizing:border-box}.page-header{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px}.page-header h1{font-size:22px;font-weight:700;color:#1a2b4a;margin:0}.btn{padding:10px 20px;border-radius:10px;font-size:14px;font-weight:600;font-family:'Plus Jakarta Sans',sans-serif;cursor:pointer;border:none;transition:all .2s;display:inline-flex;align-items:center;gap:8px}.btn-primary{background:#1a2b4a;color:#fff}.btn-primary:hover{background:#243d6a}.btn-secondary{background:#f4f6f9;color:#374151;border:1.5px solid #e5eaf0}.btn-secondary:hover{background:#e8edf2}.btn:disabled{opacity:.5;cursor:not-allowed}.toolbar{display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap}.search-box{flex:1;min-width:200px;display:flex;align-items:center;gap:8px;background:#fff;border:1.5px solid #e5eaf0;border-radius:10px;padding:0 14px}.search-box input{flex:1;border:none;outline:0;font-size:14px;padding:10px 0;background:0 0;color:#1a2b4a;font-family:'Plus Jakarta Sans',sans-serif}.search-box input::placeholder{color:#b0bcc8}.filter-select{padding:10px 14px;border:1.5px solid #e5eaf0;border-radius:10px;font-size:14px;color:#374151;background:#fff;cursor:pointer;outline:0;font-family:'Plus Jakarta Sans',sans-serif}.table-wrap{background:#fff;border-radius:14px;box-shadow:0 1px 8px rgba(0,0,0,.06);overflow:hidden}table{width:100%;border-collapse:collapse}th{text-align:left;padding:11px 16px;font-size:11px;font-weight:700;color:#7a8b9a;text-transform:uppercase;letter-spacing:.5px;background:#f8fafc;border-bottom:1px solid #e8edf2}td{padding:13px 16px;font-size:14px;color:#374151;border-bottom:1px solid #f0f4f8;vertical-align:middle}tr:last-child td{border-bottom:none}tr:hover td{background:#fafbfc}.btn-edit,.btn-revisi,.btn-export{padding:5px 12px;border:none;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;transition:background .15s;font-family:'Plus Jakarta Sans',sans-serif;display:inline-flex;align-items:center;gap:4px;text-decoration:none}.btn-edit{background:#eff6ff;color:#3b7dd8}.btn-edit:hover{background:#dbeafe}.btn-revisi{background:#fef2f2;color:#dc2626}.btn-revisi:hover{background:#fee2e2}.btn-export{background:#f0fdf4;color:#16a34a}.btn-export:hover{background:#dcfce7}.error-box{background:#fff5f5;border:1px solid #fecaca;border-radius:10px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:10px;font-size:13px;color:#dc2626;font-weight:500}.success-box{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 16px;margin-bottom:16px;font-size:14px;color:#15803d;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px}.spinner{width:16px;height:16px;border:2px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin .7s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.loading-overlay{text-align:center;padding:80px;color:#7a8b9a;font-size:14px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px}`;
...
export default function KamusPage() {
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      setUi(p => ({ ...p, msg: status === 'draft' ? 'Draft berhasil disimpan!' : 'KPI berhasil dikirim!' })); setTimeout(goBack, 1200);
    } catch (e) { setUi(p => ({ ...p, err: e.message })); } finally { setUi(p => ({ ...p, ldF: false })); }
  };
  ...
  return (
    <>
      ...
      {ui.v === 'list' && (
        <div>
          <div className="page-header">
            <h1>Kamus KPI</h1>
            <button className="btn btn-primary" onClick={() => { setForm(INIT_FORM); setUi(p => ({ ...p, v: 'tambah', stat: '', err: '' })); }}>{Ico.Add} Tambah Kamus KPI</button>
          </div>
          ...
          <div className="table-wrap">
            {ui.ldL ? <div className="loading-overlay">{Ico.Spin} Memuat data...</div> : filtered.length === 0 ? <div className="loading-overlay">{Ico.Empty}<p>{filter.q || filter.s ? 'Tidak ada hasil pencarian.' : 'Belum ada KPI. Klik "+ Tambah KPI" untuk mulai.'}</p></div> : (
              <table>
                  ...
                  <tr key={k.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <button className={k.status === 'revisi' ? 'btn-revisi' : 'btn-edit'} onClick={() => openRevisi(k.id)}>{Ico.Edit} {k.status === 'revisi' ? 'Perbaiki Revisi' : 'Revisi'}</button>
      ...
      {(ui.v === 'tambah' || ui.v === 'revisi') && (
        <div>
          <div className="page-header">
            <div><h1>{ui.v === 'revisi' ? (ui.stat === 'revisi' ? 'Perbaiki Revisi KPI' : 'Revisi Kamus KPI') : 'Tambah Kamus KPI'}</h1><p style={{ fontSize: 14, color: '#7a8b9a', margin: '4px 0 0' }}>Field bertanda <span style={{ color: '#dc2626' }}>*</span> wajib diisi.</p></div>
          ...
          {ui.ldR ? <div className="loading-overlay">{Ico.Spin} Memuat data KPI...</div> : (
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, padding: '20px 0' }}>
                ...
                <button className="btn btn-primary" onClick={() => handleSubmit('submitted')} disabled={ui.ldF}>{ui.ldF ? <><div className="spinner" /> Menyimpan...</> : (ui.stat === 'revisi' ? 'Submit' : 'Submit KPI')}</button>
              </div>
            </>
          )}
        </div>
