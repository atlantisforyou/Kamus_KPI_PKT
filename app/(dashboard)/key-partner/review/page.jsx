...
export default function KeyPartnerReviewPage() {
  ...
  const [flt, setFlt]   = useState({ s: '' }); // Menghapus filter query 'q'
  ...
  const fltd = list.filter(k => !flt.s || k.status === flt.s);
  
  const c = { 
    drf: list.filter(k => k.status === 'draft').length, 
    sub: list.filter(k => k.status === 'submitted').length,
    rev: list.filter(k => k.status === 'reviewed').length,
    rvs: list.filter(k => k.status === 'revisi').length, 
    app: list.filter(k => k.status === 'approved').length 
  };
    ...
    <>
      ...
      <div>
        ...
        <div className="tab-bar">
          {[
            { k: '', l: 'Semua', c: list.length }, 
            { k: 'draft', l: 'Draft', c: c.drf }, 
            { k: 'submitted', l: 'Butuh Review', c: c.sub }, 
            { k: 'reviewed', l: 'Reviewed', c: c.rev }, 
            { k: 'revisi', l: 'Revisi', c: c.rvs }, 
            { k: 'approved', l: 'Approved', c: c.app }
          ].map(t => (
            <button key={t.k} className={`tab ${flt.s === t.k ? 'active' : ''}`} onClick={() => setFlt({ s: t.k })}>{t.l}<span className="tab-count">{t.c}</span></button>
          ))}
        </div>

        <div className="table-wrap">
          {load ? <div className="loading">{Ico.Load} Memuat data...</div> : 
            fltd.length === 0 ? <div className="empty"><div>{Ico.Empty}</div><p>Tidak ada data KPI yang sesuai filter.</p></div> : 
          (
            <table>
                  ...
                  <tr key={k.id}>
                    ...
                    <td>
                      <div className="btn-action-group">
                        ...
                        {k.status !== 'approved' && k.status !== 'revisi' && (
