...
const Fld = ({ l, val, onChange, t = "text", p, max, st, autoF, isPass, show, onToggle }) => (
  <div className="field">
    ...
    <div style={{ position: 'relative' }}>
      <input 
        type={isPass ? (show ? "text" : "password") : t} 
        placeholder={p} 
        value={val} 
        onChange={onChange} 
        maxLength={max} 
        style={{ ...st, paddingRight: isPass ? '42px' : '16px' }} 
        autoFocus={autoF} 
        autoComplete="off" 
      />
      {isPass && (
        <button 
          type="button" 
          onClick={onToggle} 
          style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#7a8b9a', display: 'flex', padding: '4px' }}
        >
                ...
                <form onSubmit={actions.login}>
                  ...
                  {isAdmin && (
                    <Fld 
                      l="Password" 
                      val={f.loginPass} 
                      onChange={(e) => upd('loginPass', e.target.value)} 
                      p="Masukkan password admin" 
                      isPass={true} 
                      show={showP} 
                      onToggle={() => setShowP(!showP)} 
                    />
                  )}
